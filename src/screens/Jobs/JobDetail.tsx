  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setJob(data || null);
        // check if an assessment exists for this job
        try {
          const a = await assessmentService.getAssessmentByJobId(jobId);
          setHasAssessment(!!a);
        } catch (e) {
          console.error('Failed to check assessment for job:', e);
        }
      } catch (e) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {hasAssessment ? (
            ...
            <Button variant="primary" onClick={() => navigate(`/assessments/${job.id}/builder`)} className="inline-flex items-center gap-2">
